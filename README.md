...
[![Deploy](https://www.herokucdn.com/deploy/button.png)](https://dashboard.heroku.com/new?template=https://github.com/qw21aswewa/mei)  
...
4）然后修改app.json文件中的`name`、`description`�?
5）再修改`Deploy to Heroku`按键指向地址为自己仓库地址，重复`2）`的操�?
## vmess vless trojan-go shadowsocks对应客户端参数的参考如�?末尾�?)里的内容仅为提示
...
* 端口�?43
* 默认UUID�?f91b6a0-e8ee-11ea-adc1-0242ac120002   (务必创建时自定义UUID�?
* 加密：none
* 传输协议：ws
* 伪装类型：none
...
* 服务器地址：自选ip（如：icook.tw）或者：应用程序�?herokuapp.com
* 端口�?43
* 密码�?f91b6a0-e8ee-11ea-adc1-0242ac120002   (务必创建时自定义UUID�? 
* 传输协议：ws
* path路径�?自定义UUID�?trojan  (注意：前有斜�?)
* SNI地址�?***.workers.dev(CF Workers反代地址)或者：应用程序�?herokuapp.com
* 伪装host�?***.workers.dev(CF Workers反代地址)或者：应用程序�?herokuapp.com

## 3：Shadowsocks+ws+tls

* 服务器地址: 应用程序�?herokuapp.com
* 端口: 443
* 密码�?f91b6a0-e8ee-11ea-adc1-0242ac120002   (务必创建时自定义UUID�? 
* 加密：chacha20-ietf-poly1305
* 插件选项: tls;host=应用程序�?herokuapp.com;path=/自定义UUID�?ss


### CloudFlare Workers反代代码（支持VLESS\VMESS\Trojan-Go的WS模式，可分别用两个账号的应用程序名（UUID与path保持一致），单双号天分别执行，那一个月就有550+550小时�?
```
const SingleDay = '应用程序�?.herokuapp.com'
const DoubleDay = '应用程序�?.herokuapp.com'
addEventListener(
    "fetch",event => {
